...
def run_batched_mode(video_paths, duration=30):
    print("\n--- Running Day Mode A: Threaded + Batched Inference ---")
...
def run_multiprocessing_mode(video_paths, duration=30):
    print("\n--- Running Day Mode B: Multiprocessing (Independent) ---")
...
def run_queue_mode(video_paths, duration=30):
    print("\n--- Running Day Mode C: Thread-based Pipeline with Queues ---")
...
if __name__ == "__main__":
    ...
    print_report("DAY MODE A (Threaded + Batched)", f_a, t_a, m_a, u_a, mem_a)
    
    f_b, t_b, m_b, u_b, mem_b = run_multiprocessing_mode(VIDEO_FILES, TEST_DURATION)
    print_report("DAY MODE B (Multiprocessing)", f_b, t_b, m_b, u_b, mem_b)
    
    f_c, t_c, m_c, u_c, mem_c = run_queue_mode(VIDEO_FILES, TEST_DURATION)
    print_report("DAY MODE C (Threaded Queues)", f_c, t_c, m_c, u_c, mem_c)
