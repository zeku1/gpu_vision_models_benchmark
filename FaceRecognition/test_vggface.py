...
# Force DeepFace to build the cache file (verbose=False removed to prevent crashes)
try: 
...
if not cap.isOpened():
    source = "Webcam" if USE_WEBCAM else "RTSP stream"
    print(f"Error: Could not open {source}.")
    exit()

cv2.namedWindow("Tapo - Security", cv2.WINDOW_NORMAL)
...
while True:
    ...
    cv2.imshow("Tapo - Security", res_image)
