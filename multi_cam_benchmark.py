...
if not VIDEO_FILES:
    print("Error: No video samples found. Run download_samples.py first.")
    exit(1)
...
class VisionPipeline:
    def __init__(self, device='cuda' if torch.cuda.is_available() else 'cpu'):
        print(f"Initializing VisionPipeline on {device}...")
        self.device = device
        ...
        self.easy_reader = easyocr.Reader(['en'], gpu=(device == 'cuda'))
...
def run_batched_mode(video_paths, duration=30):
    print("\n--- Running Mode A: Threaded + Batched Inference ---")
...
def run_multiprocessing_mode(video_paths, duration=30):
    print("\n--- Running Mode B: Multiprocessing (Independent) ---")
...
def run_queue_mode(video_paths, duration=30):
    print("\n--- Running Mode C: Thread-based Pipeline with Queues ---")
    ...
    consumers = [threading.Thread(target=consumer) for _ in range(2)] # 2 consumers
...
def print_report(name, total_frames, total_time, metrics):
    ...
    avg_v8_conf = np.mean([m['v8_avg_conf'] for m in metrics])
    avg_v10_conf = np.mean([m['v10_avg_conf'] for m in metrics])
    avg_v8_count = np.mean([m['v8_count'] for m in metrics])
    avg_v10_count = np.mean([m['v10_count'] for m in metrics])
    
    print(f"\nRESULTS FOR {name}:")
    print(f"  Total Frames: {total_frames}")
    print(f"  Overall FPS:  {fps:.2f}")
    print(f"  Avg Latency:  {avg_latency:.1f}ms")
    print(f"  Accuracy Proxy (Confidence/Detections):")
    print(f"    YOLOv8 (Vehicles): {avg_v8_conf:.2f} conf, {avg_v8_count:.1f} obj/frame")
    print(f"    YOLOv10 (Person):  {avg_v10_conf:.2f} conf, {avg_v10_count:.1f} obj/frame")
...
if __name__ == "__main__":
    ...
    print_report("MODE A (Threaded + Batched)", f_a, t_a, m_a)
    
    f_b, t_b, m_b = run_multiprocessing_mode(VIDEO_FILES, TEST_DURATION)
    print_report("MODE B (Multiprocessing)", f_b, t_b, m_b)
    
    f_c, t_c, m_c = run_queue_mode(VIDEO_FILES, TEST_DURATION)
    print_report("MODE C (Threaded Queues)", f_c, t_c, m_c)
